...
const Add_New_Booking = () => {
 const navigate=useNavigate()
  ...
  const handleuploadImage=async(e)=>{
    ...
    const getdata=await uploadImage(file)
    setData((preve)=>{
      return {
        ...preve,
        orderImage:getdata
  ...
  const createbooking=async()=>{
    ...
    console.log('create booking data',resdata)
  if(resdata.success){
  ...
  <div className="w-96 mt-4 flex flex-col gap-5">
    <div className="flex">
      <select onChange={handleOnChange} name='orderNo' value={data.orderNo} className="h-11 w-full px-2 border border-gray-400 rounded-lg outline-none">
    ...
    <div className="flex flex-col">
      ...
      <input onChange={handleOnChange} name='bookingDate' value={data.bookingDate} type="date" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>

    <div className="flex">
      <input onChange={handleOnChange} name='partyName' value={data.partyName} type="number" placeholder="Process Loss" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>

    <div className="flex">
      <input onChange={handleOnChange} name='rib' value={data.rib} type="text" placeholder="Rib" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>
  </div>
  ...
  <div className="w-96 mt-4 flex flex-col gap-5">
    <div className="flex">
      <input onChange={handleOnChange} name='preparedBy' value={data.preparedBy} type="text" placeholder="Prepared By" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>
    <div className="flex">
      <input onChange={handleOnChange} name='composition' value={data.composition} type="text" placeholder="Composition" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>
    <div className="flex">
      <input onChange={handleOnChange} name='otherFabric' value={data.otherFabric} type="text" placeholder="Others Fabric" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>
    <div className="flex">
      <input onChange={handleOnChange} name='collar' value={data.collar} type="text" placeholder="Collar" className="h-10 w-full px-2 border border-gray-400 rounded-lg outline-none" />
    </div>
  </div>

  {/* Third Column */}
  <div className="w-96 mt-4 flex flex-col gap-5  left-[63rem] ms-3 p-2">
    <div className="flex relative">
      <label htmlFor="upload" className="flex flex-col  items-center justify-center cursor-pointer border-2  rounded-lg p-2">
        ...
        <input onChange={handleuploadImage} type="file" name="image" id="upload" accept="image/*" className="hidden" />
    ...
    <Link to={'/bookinglist'} className=' btn btn-join w-48 h-11 bg-white' style={{borderRadius:"5px",border:"1px solid black"}}>Cancel</Link>
    <button onClick={createbooking} className='w-48 h-11 text-white bg-purple-500' style={{borderRadius:"5px"}}>Save</button>
